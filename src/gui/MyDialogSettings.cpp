...
wxString MyDialogSettings::getClientMode() const
{
  if(clientChoice->GetSelection() == 0)
    return V_RCONLY;
  if(clientChoice->GetSelection() == 1)
    return V_VIEWER;

  return wxEmptyString;
}

wxString MyDialogSettings::getRConly() const
{
  return customClientRConly->GetValue();
}



wxString MyDialogSettings::getViewer() const
{
  return customClientViewer->GetValue();
}
